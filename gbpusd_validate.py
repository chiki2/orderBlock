"""
GBPUSD Winner Validation — tests best config across year windows.
Winner: STOP orders (PF 1.04, DD 3.6%, 61% WR)
Usage: python3 gbpusd_validate.py
"""
...
RESULTS_MD = f"{BASE_DIR}/docs/gbpusd_validation.md"
...
def main():
    print(f"\n{'='*55}")
    print(f"  GBPUSD Winner Validation — {len(WINDOWS)} windows")
    print(f"  Config: STOP orders (best from 16-test sweep)")
    print(f"{'='*55}\n")
    ...
    print("  Loaded gbpusd_best.set → gbpusd.set\n")

    header = f"""# GBPUSD Winner Validation
