#!/usr/bin/env python3
"""
Re-run GBPUSD ATR sweep tests that failed due to compile issue.
"""
...
def main():
    total = len(ATR_TESTS)
    print(f"\n{'='*65}", flush=True)
    print(f"  GBPUSD ATR Re-run -- {total} tests", flush=True)
    print(f"{'='*65}\n", flush=True)

    csv_rows = []

    for idx, (label, overrides) in enumerate(ATR_TESTS, 1):
        ts = datetime.now().strftime("%H:%M")
        print(f"\n[{idx}/{total}] {ts} -- GBPUSD {label}", flush=True)

        actual_set = apply_overrides("gbpusd.set", overrides, "GBPUSD")

        t0 = time.time()
        result = run_backtest("GBPUSD", actual_set)
        ...
        csv_rows.append({
            "symbol": "GBPUSD", "label": label, "pf": pf,
    ...
    csv_file = "gbpusd_atr_results.csv"
    if csv_rows:
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  GBPUSD ATR RESULTS", flush=True)
