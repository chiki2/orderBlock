#!/usr/bin/env python3
"""
XAUUSD cross-year validation: Option B (D1-Candle) vs Option C (No-DailyBias).

Runs 5 time windows × 2 configs = 10 backtests.
...
def apply_overrides(overrides, tag):
    path = os.path.join(BASE, "OBInclude", "SetFiles", "claude.set")
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  XAUUSD CROSS-YEAR VALIDATION: Option B vs Option C", flush=True)
    ...
    # Summary
    csv_file = "xauusd_crossyear_results.csv"
    ...
    print(f"\n  {'Config':<22} {'2022':>7} {'2023':>7} {'2024':>7} {'2025':>7} {'Full PF':>8}", flush=True)
