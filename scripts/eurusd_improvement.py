#!/usr/bin/env python3
"""
EURUSD improvement sweep — test filters that worked on other symbols.
Current baseline: PF=1.14 (not deployable)
"""
...
TESTS = [
    ("Baseline", {}),
    # D1 Trend filter — biggest potential (works on USDJPY, EURJPY, GBPUSD)
    ("D1Trend=true", {"inpRequireD1Trend": "true||false||0||true||N"}),
    # forbidMondayFriday — dominant for EURJPY/GBPUSD
    ("NoMonFri", {"forbidMondayFriday": "true||false||0||true||N"}),
    # D1Trend + NoMonFri combo
    ("D1+NoMonFri", {
        "inpRequireD1Trend": "true||false||0||true||N",
        "forbidMondayFriday": "true||false||0||true||N",
    }),
    # outdatedOB variants (40 was winner for USDJPY)
    ("OB40", {"outdatedOB": "40||80||1||200||N"}),
    ("OB120", {"outdatedOB": "120||80||1||200||N"}),
    # D1Trend + OB40
    ("D1+OB40", {
        "inpRequireD1Trend": "true||false||0||true||N",
        "outdatedOB": "40||80||1||200||N",
    }),
    # STOP orders (works for EURJPY)
    ("STOP", {"typeofOrder": "2||1||0||2||N"}),
    ("STOP+D1", {
        "typeofOrder": "2||1||0||2||N",
        "inpRequireD1Trend": "true||false||0||true||N",
    }),
    # TP variations
    ("TP=1.5", {"fibo1rstTP": "1.5||1.27||0.100000||3.000000||N"}),
    ("TP=2.0", {"fibo1rstTP": "2.0||1.27||0.100000||3.000000||N"}),
    ("TP=3.0", {"fibo1rstTP": "3.0||1.27||0.100000||3.000000||N"}),
    # D1 + TP combos
    ("D1+TP1.5", {
        "inpRequireD1Trend": "true||false||0||true||N",
        "fibo1rstTP": "1.5||1.27||0.100000||3.000000||N",
    }),
    ("D1+TP3.0", {
        "inpRequireD1Trend": "true||false||0||true||N",
        "fibo1rstTP": "3.0||1.27||0.100000||3.000000||N",
    }),
    # Skip09UTC (London open death zone)
    ("Skip09", {"inpSkip09UTC": "true||false||0||true||N"}),
    # BlockCounterHTF
    ("BlockCHTF", {"inpBlockCounterHTF": "true||false||0||true||N"}),
    # DailyBias
    ("DailyBias", {"inpDailyBiasEnabled": "true||false||0||true||N"}),
    # Kitchen sink: D1 + NoMonFri + OB40
    ("D1+NoMonFri+OB40", {
        "inpRequireD1Trend": "true||false||0||true||N",
        "forbidMondayFriday": "true||false||0||true||N",
        "outdatedOB": "40||80||1||200||N",
    }),
    # Tolerance variations (EURUSD is 5-digit, 50pts = 5 pips)
    ("tol=80", {"tolerance": "80||50||1||200||N"}),
    ("tol=30", {"tolerance": "30||50||1||200||N"}),
]
...
def main():
    total = len(TESTS)
    print(f"\n{'='*65}", flush=True)
    print(f"  EURUSD IMPROVEMENT SWEEP -- {total} tests", flush=True)
    print(f"{'='*65}\n", flush=True)
    ...
    csv_file = "eurusd_improvement.csv"
    if csv_rows:
    ...
    # Summary
    print(f"\n{'='*65}", flush=True)
    print(f"  EURUSD IMPROVEMENT RESULTS (baseline PF={baseline_pf:.2f})", flush=True)
    print(f"{'='*65}", flush=True)
    print(f"  {'Config':<25} {'PF':<8} {'Trades':<8} {'DD%':<8} {'Delta':<8}", flush=True)
    for r in csv_rows:
        delta = r["pf"] - baseline_pf if baseline_pf > 0 else 0
        marker = "+" if delta > 0 else " " if delta == 0 else ""
        print(f"  {r['label']:<25} {r['pf']:<8.2f} {r['trades']:<8} {r['dd_pct']:<8.2f} {marker}{delta:<+.2f}", flush=True)

    # Winners
    print(f"\n  TOP IMPROVEMENTS:", flush=True)
    winners = [(r["label"], r["pf"], r["pf"] - baseline_pf) 
               for r in csv_rows if r["pf"] > baseline_pf and r["trades"] >= 5 and r["label"] != "Baseline"]
    winners.sort(key=lambda x: -x[2])
    for label, pf, delta in winners[:5]:
        print(f"    {label:<25} PF={pf:.2f}  (+{delta:.2f})", flush=True)
    if not winners:
