#!/usr/bin/env python3
"""Quick cross-year revalidation for GBPUSD with tp=1.5 applied."""
...
def main():
    print(f"\n{'='*65}", flush=True)
    print(f"  GBPUSD Cross-Year (tp=1.5) -- 5 windows", flush=True)
    print(f"{'='*65}\n", flush=True)
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  GBPUSD Summary (tp=1.5)", flush=True)
    ...
    # Compare with previous (tp=1.27)
    prev = [(1.52, 26), (1.53, 27), (1.32, 8), (0.59, 11), (14.53, 0)]
    print(f"\n  Comparison (old tp=1.27 -> new tp=1.5):", flush=True)
    for i, ((label, pf, trades, dd), (prev_pf, prev_tr)) in enumerate(zip(results, prev)):
        delta_pf = pf - prev_pf
        print(f"  {label:<12} PF: {prev_pf:.2f} -> {pf:.2f} ({delta_pf:+.2f})", flush=True)
