#!/usr/bin/env python3
"""
EURJPY trailing stop improvement sweep (Item 2).

Tests fibo1rstTP = 1.5 (baseline), 2.0, 3.0 in cross-year windows.
EURJPY current config already uses Classic trailing (trailingStrat=0).
Hypothesis: raising fibo1rstTP gives more room before partial close,
matching the USDJPY winner config (fibo1rstTP=3.0 → PF=3.07).
...
CONFIGS = [
    ("fibo1rst=1.5 (baseline)", {"fibo1rstTP": "1.5||1.0||0.1||1.8||N"}),
    ("fibo1rst=2.0",            {"fibo1rstTP": "2.0||1.0||0.1||1.8||N"}),
    ("fibo1rst=3.0",            {"fibo1rstTP": "3.0||1.0||0.1||1.8||N"}),
]
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  EURJPY TRAILING STOP SWEEP (Item 2)", flush=True)
    ...
    for cfg_label, overrides in CONFIGS:
        ...
        tag = cfg_label.split("=")[1].split()[0].replace(".", "_")
        ...
        verdict = ok_years >= 3 and full and full["pf"] >= 1.5
        print(f"  VERDICT: {'PASS ✓' if verdict else 'FAIL ✗'}", flush=True)

    csv_file = "eurjpy_trailing_sweep_results.csv"
    ...
    print(f"\n\n{'='*70}", flush=True)
    print(f"  FINAL SUMMARY — EURJPY TRAILING SWEEP", flush=True)
