#!/usr/bin/env python3
"""
USDJPY tolerance sweep — can we improve the best symbol further?
Current: 5/5 cross-year, PF=1.57 (full range). tp=3.0 already applied.
"""
...
TESTS = [
    ...
    ("tp=2.0", {"fibo1rstTP": "2.0||1.27||0.127000||12.700000||N"}),
    ("tp=2.0 + tol=80", {
        "fibo1rstTP": "2.0||1.27||0.127000||12.700000||N",
...
CSV_FILE = "usdjpy_tol_results.csv"
...
def main():
    total = len(TESTS)
    print(f"\n{'='*65}", flush=True)
    print(f"  USDJPY Tolerance Sweep -- {total} tests", flush=True)
    print(f"{'='*65}\n", flush=True)
    ...
    for idx, (label, overrides) in enumerate(TESTS, 1):
        ...
        actual_set = apply_overrides("usdjpy.set", overrides)
        ...
        csv_rows.append({
            "label": label, "pf": pf, "trades": trades, "dd_pct": dd,
            "log": diag["summary"],
        })

        tmp_path = os.path.join(BASE, "OBInclude", "SetFiles", "_tmp_usdjpy.set")
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  USDJPY SWEEP RESULTS", flush=True)
    print(f"{'='*65}", flush=True)
    print(f"  {'Label':<25} {'PF':<8} {'Trades':<8} {'DD%':<8}", flush=True)
    for r in csv_rows:
