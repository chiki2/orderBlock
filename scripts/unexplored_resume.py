#!/usr/bin/env python3
"""
Resume unexplored sweep from where it died + re-run USDJPY failed tests (1-4).
Pre-seeds known results from the partial run.
"""
...
# Pre-seed known results from partial run (tests 5-9 USDJPY succeeded)
KNOWN = {
    ("USDJPY", "No Mon/Fri"): {"pf": 1.44, "trades": 27, "dd_pct": 1.69, "balance": 10000},
    ("USDJPY", "outdatedOB=40"): {"pf": 1.86, "trades": 34, "dd_pct": 2.00, "balance": 10000},
    ("USDJPY", "outdatedOB=120"): {"pf": 1.55, "trades": 42, "dd_pct": 2.01, "balance": 10000},
    ("USDJPY", "outdatedOB=200"): {"pf": 1.46, "trades": 44, "dd_pct": 2.01, "balance": 10000},
    ("USDJPY", "maxWick=1.2"): {"pf": 1.63, "trades": 39, "dd_pct": 1.97, "balance": 10000},
}
...
def main():
    total_to_run = sum(1 for s, _ in SYMBOLS for l, _ in TESTS if (s, l) not in KNOWN)
    total_all = len(SYMBOLS) * len(TESTS)
    print(f"\n{'='*70}", flush=True)
    print(f"  UNEXPLORED SWEEP RESUME -- {total_to_run} tests to run ({len(KNOWN)} cached)", flush=True)
    print(f"{'='*70}\n", flush=True)
    ...
    for symbol, set_file in SYMBOLS:
        ...
        for label, overrides in TESTS:
            ...
            if key in KNOWN:
                r = KNOWN[key]
                print(f"\n  [cached] {symbol} {label} -> PF={r['pf']:<8.2f} Trades={r['trades']}", flush=True)
                if label == "Baseline":
                    baselines[symbol] = r["pf"]
                csv_rows.append({
                    "symbol": symbol, "label": label, "pf": r["pf"],
                    "trades": r["trades"], "dd_pct": r["dd_pct"], "balance": r["balance"],
                })
                continue

            run_num += 1
            ts = datetime.now().strftime("%H:%M")
            print(f"\n[{run_num}/{total_to_run}] {ts} -- {symbol} {label}", flush=True)
