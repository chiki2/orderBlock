#!/usr/bin/env python3
"""
EURJPY combo sweep — test tolerance + D1Trend + MacroTrend combos.
Base: STOP orders + fibo1rstTP=1.5 (current config).
EURJPY is 3/5 cross-year profitable. 2023 and 2024 are losing years.
Goal: find config that improves 2023-2024 without hurting 2022/2025/2026.
"""
...
TESTS = [
    ("Baseline", {}),
    ("tolerance=80", {"tolerance": "80||50||1||500||N"}),
    ("tolerance=100", {"tolerance": "100||50||1||500||N"}),
    ...
    ("tol=100 + No D1", {
        "tolerance": "100||50||1||500||N",
        "inpRequireD1Trend": "false||false||0||true||N",
    }),
    ("LIMIT orders (keep tp=1.5)", {
        "typeofOrder": "1||1||0||2||N",
    }),
    ("LIMIT + tol=80", {
        "typeofOrder": "1||1||0||2||N",
        "tolerance": "80||50||1||500||N",
...
CSV_FILE = "eurjpy_combo2_results.csv"
...
def main():
    total = len(TESTS)
    print(f"\n{'='*65}", flush=True)
    print(f"  EURJPY Combo Sweep 2 -- {total} tests", flush=True)
    print(f"{'='*65}\n", flush=True)
    ...
    for idx, (label, overrides) in enumerate(TESTS, 1):
        ...
        actual_set = apply_overrides("eurjpy.set", overrides)
        ...
        tmp_path = os.path.join(BASE, "OBInclude", "SetFiles", "_tmp_eurjpy.set")
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  EURJPY COMBO SWEEP 2 RESULTS", flush=True)
    print(f"{'='*65}", flush=True)
    print(f"  {'Label':<35} {'PF':<8} {'Trades':<8} {'DD%':<8}", flush=True)
    for r in csv_rows:
