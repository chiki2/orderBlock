#!/usr/bin/env python3
"""
GBPUSD trailing stop improvement sweep.

GBPUSD current: trailingStrat=0 (Classic), fibo1rstTP=1.5, MinRR=1.0
Tests fibo1rstTP = 1.5 (baseline), 2.0, 3.0
USDJPY won with fibo1rstTP=3.0 (+1.16 PF). Test if same applies to GBPUSD.
...
def main():
    print(f"\n{'='*70}", flush=True)
    print(f"  GBPUSD TRAILING STOP SWEEP", flush=True)
    ...
    csv_file = "gbpusd_trailing_sweep_results.csv"
    ...
    print(f"\n\n{'='*70}", flush=True)
    print(f"  FINAL SUMMARY — GBPUSD TRAILING SWEEP", flush=True)
